...
void skipSpace()
{
    ...
    c = getchar();
    line_no += (c == '\n');
	//skip the character if it is a space && eof is not reach
    ...
    while (pc != NULL)
    {
        switch (pc->type)
        {
            ...
            case ASSIGN_STMT:
                ...
                debug("%s: %s=%d\n", "=", pc->assign_stmt->left_hand_side->name, result);
            ...
            case IF_STMT:
                ...
                debug("%s: %s=%d, %s=%d\n", "if/while",
                      pc->if_stmt->condition_operand1->name, op1,
                      pc->if_stmt->condition_operand2->name, op2);
